...
- **OCR backend** (`ocr.backend`): selects `paddleocr`, `surya`, `auto`, or `disabled` for the pre-pass before `extract_fields_vision`. PaddleOCR settings live under `ocr.paddleocr`.
- **OCR injection cap** (`agent.ocr_prompt_max_chars`): truncates OCR text passed into `extract_fields_vision` prompts (full-page and crop paths).
